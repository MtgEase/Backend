...
import os
import yaml
from Database.StorageDriver.MySql import util
...
class MySql:
    ...
    def __init__(self, **config):
        ...
        try:
            ...
            logger.info(f'[Database/MySql] Connected to MySql!')
        ...
        try:
            self.__check_tables()
            logger.info(f'[Database/MySql] Database struct OK!')
        except (AssertionError, pymysql.err.MySQLError):
            logger.error(f'[Database/MySql] Database struct ERROR!')
            ...
            if os.system(f'mysqldump -h {self.host} -P {self.port} -u {self.user} -p {self.passwd} {self.database} > '
                         f'{backup_file}') != 0:
                logger.error(f'[Database/MySql] "mysqldump" DID NOT run correctly. Have you already installed it? '
                             f'(Return non-zero exit code)')
                exit(1)
            logger.info(f'[Database/MySql] Corrupt database backed at: {backup_file}')
            if self.config['init_database_confirm']:
                try:
                    ...
                    logger.error(f'[Database/MySql] CAN NOT write config file: Config/config.yaml\n\t{str(e)}')
                    exit(1)
                try:
                    self.__init_tables()
                except pymysql.err.MySQLError as e:
                    logger.error(f'[Database/MySql] CAN NOT connect to MySql!\n\t{str(e)}')
                    exit(1)
                logger.info(f'[Database/MySql] Database inited.')
            else:
                logger.info(f'[Database/MySql] Exiting...')
                exit(1)
    ...
    def __init_tables(self):
        """初始化表结构"""
        for file in os.listdir('Database/StorageDriver/MySql/SqlMap/'):
            if file.endswith('.sql'):
                with open(os.path.join('Database/StorageDriver/MySql/SqlMap/', file), 'r', encoding='utf-8') as f:
                    sql = f.readlines()
                    correct_sql = []
                    temp_sql = ''
                    for line in sql:
